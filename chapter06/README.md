...
**Harness 视角**：这一章添的是边界，也是整套挽具里人和机器唯一正式碰面的地方。给模型一双手之前，先得回答几个问题：它能碰哪儿（工作目录）、能跑多久（超时）、哪些命令绝不许（黑名单）；至于"这一次到底动不动手"，交给 `interrupt_on`，由你按下那一下。这四道闸门都长在模型外面，它既看不见，也绕不过去。把能力放出来，同时把破坏力关在门里，这是挽具存在的另一半理由。
