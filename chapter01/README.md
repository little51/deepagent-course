...
本章就干这件事，分两部分：先把工作台搭起来（git bash 敲命令、uv 管环境、DeepSeek 作为模型支持），再写出第一个能用的命令行程序——十行代码，先只实现对话，它叫 minihermes。学完这一章应该能自己复现一套相同的环境，让程序在终端里连着跟你聊天，并且明白四件事：模型其实什么都不记得、"聊天记录"是怎么来的、为什么说智能体只是"一个循环加一份上下文"，以及 Harness 这层工程到底管什么。后面五章要加的所有东西，都得从这个十行的内核里长出来。
...
harness 这层工程是 2026 年 2 月才被正式命名的。HashiCorp 的创始人 Mitchell Hashimoto 在一篇讲他自己怎么用 AI 的博客里给了它一个用法，大意是：每次你发现智能体犯了错，就花点时间把它干活的环境改一改，让它再也不可能犯同一个错。他把这套做法叫 harness engineering。

六天之后，OpenAI 的 Ryan Lopopolo 发了一篇同名的文章，把一件更狠的事摆了出来。他们团队从空仓库起步，五个月做了一个内部产品：大约一百万行代码、一千五百来个 Pull Request，三个人起头，后来七个人。这个项目里没有一行代码是手写的，全由 Codex 写。作者把这件事浓缩成一句话：人类掌舵，智能体执行。

照那篇文章的说法，工程师干的活变了：写代码退到一边，主要工作成了设计环境、把意图说清楚、搭好反馈回路，好让智能体可靠地干活。早期进展慢，卡在环境这一头：智能体缺工具、缺抽象、缺能用的内部结构。所以进度卡住的时候，他们的问法不是"再努力一点"，而是"还缺什么能力，这个能力怎么才能对智能体既清晰可读、又能强制执行"。

文章里我印象最深的一条：要给智能体一张地图，不要给它一本一千页的说明书。他们一开始试过把 `AGENTS.md` 写成大百科，很快就烂掉了——上下文是稀缺资源，什么都"重要"就等于什么都不重要。最后那份文件只剩一百来行，只当地图用，真正的知识放进了结构化的 `docs/` 目录。这一条等到第 4 章讲技能、第 5 章讲记忆时还会碰到。

### 1.4 它管哪些事

把这几家的做法放到一起看，harness 干的事可以归成两条法则和四件事。

两条法则是：能用确定性的办法卡死的地方，就别靠提示词去求它；以及，智能体要能从环境里拿到"地面真相"，能自己判断做没做对、错了能回头。前一条对应代码里的类型检查、lint、沙盒、CI 的硬失败；后一条对应"跑一遍看结果"这种最朴素的验证。圈里还有个说法：能不能用智能体真把活干成，跟它能不能验证自己的活强相关。

四件事是：

| 管什么 | 具体指什么 |
| --- | --- |
| 上下文 | 给地图，不给百科全书；要细节的时候再让它自己去读 |
| 工具与权限 | 哪些事能干、从哪个接口干（MCP 是通用接口之一）、哪些操作必须先问人 |
| 状态 | 进度落到文件里，不必全靠一张越聊越长的上下文 |
| 执行 | 在隔离的环境里跑，跑完能自己看结果 |

对照本书六章，这四件事正好被拆开讲了一遍：第 2 章把循环和上下文托管起来，第 3 章自己写工具，第 4 章用技能回答"上下文怎么省着给"，第 5 章把记忆落到磁盘上，第 6 章补上执行环境和四道闸门。所以这本书从头到尾只做一件事：给它加挽具。
