...
前几天我让 Hermes 帮我查一件事。它自己搜了一圈，读了几页网页，在工作目录里写下笔记，最后交给我一份带出处的总结。中间还停下来问了一句：这条命令要执行吗？看它跑完，我脑子里冒出来的是另一个问题：如果从零开始写这么一个东西，第一行该敲什么。

本章就干这件事，分两部分：先把工作台搭起来（git bash 敲命令、uv 管环境、DeepSeek 出脑子），再写出第一个能用的命令行程序——十行代码，只会说话，它叫 minihermes。学完这一章你应该能自己复现一套相同的环境，让程序在终端里连着跟你聊，并且明白三件事：模型其实什么都不记得、"聊天记录"是怎么来的、为什么说智能体只是"一个循环加一份上下文"。后面五章要加的所有东西，都得从这个十行的内核里长出来。
...
本书的主角 DeepAgent（Python 包名 `deepagents`）就是 LangChain 官方做的一个 Harness。它是个独立的包，底下站在 LangChain 和 LangGraph 上，把下面这些枯燥但又必须的事都办了：
...
有件事得先说清楚，不然到第二章会犯迷糊：deepagents 是个库，不是命令行程序。

我验过：装完之后 `.venv/Scripts` 里没有任何可执行文件，包也没注册任何命令。`create_deep_agent(...)` 交给你的是一张编译好的 LangGraph 图，剩下的事情得你自己来。读输入、打印输出、`你 > ` 这个提示符、斜杠命令，全都不在包里。
...
| 章 | 这一章给 minihermes 加什么本事 |
| --- | --- |
| 1 | 开发环境搭建与第一个程序：十行，只会说话 ← 本章 |
| 2 | 用 DeepAgent 开发最简智能体：会读写文件、会自己拆步骤 |
| 3 | 工具调用：给智能体接入联网能力 |
| 4 | 技能（Skills）：把做事的方法固化成能力 |
| 5 | 记忆（Memory）：让智能体跨会话记住用户 |
| 6 | 程序执行与沙盒：让智能体安全地动手干活 |
...
**git bash。** Windows 自带的 cmd 和 PowerShell 写脚本总差点意思：路径是反斜杠，管道经常不灵，中文还爱乱码。所以本书统一用 git bash。装个 Git for Windows（<https://git-scm.com/download/win>，一路默认），装完在开始菜单里找 Git Bash，验一下：
...
第六行开始主循环：打印 `你 > `，等你敲一行。空行、`exit`、`quit` 都算告辞。`:=` 是海象运算符，边赋值边判断，能省一行。
...
![第 1 章：在 Windows Terminal 里跑出来的结果](images/运行结果.png)

*上面这张是 wt（Windows Terminal）里的原样截图，不是手抄的。管道方式运行时输入不会回显，所以 `你 > ` 和回答挨在一行，不是排版错了。*
...
下一章，命令行还是今天这个命令行，我们往里边装 DeepAgent。装完之后 minihermes 会长出文件工具、任务规划和子智能体，也会第一次在磁盘上留下东西：一个叫 `workspace/` 的目录，里面躺着它自己写的笔记。
