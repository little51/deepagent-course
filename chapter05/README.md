...
*git bash 窗口里的原样截图：上半段是第一个进程（它把话记进 `AGENTS.md`），下半段是**新起的第二个进程**在回答"我是谁"。*
...
**Harness 视角**：这一章添的是记性，而且是挽具的记性。模型本身没记性，换个进程它就谁也不认识；能认出人，靠的是挽具在开机时把 `memories/AGENTS.md` 读进上下文、聊天中让模型自己去更新这份文件。会话里的记忆放在 `checkpointer`，跨会话的记忆落在磁盘上，两份东西的分工在这一章讲清楚了。记忆也是这一层里最该防着的一块：今天写进去的备注，明天会被当成指令读回来，所以多用户场景下它的写入权限要收紧。
